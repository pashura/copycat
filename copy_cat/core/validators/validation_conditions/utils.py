...
def get_successful_conditions_count(
    conditions: list[Condition], test_data: list[DataObject], location: str
) -> int:
    count = 0
    for condition in conditions:
        test_data_obj = get_test_data_object(
            test_data, f"{location}/{condition.element}"
        )
