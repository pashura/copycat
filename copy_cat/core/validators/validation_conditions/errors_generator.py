...
class ErrorsGenerator:
    ...
    @staticmethod
    def _generate_if_then_error(
        validation: ValidationCondition, location: str
    ) -> Error:
        ...
        error_message = (
            f"If {conditions_conjunction.join(conditions_elements)}, "
    ...
    @staticmethod
    def _generate_at_least_one_of_error(
        validation: ValidationCondition, location: str
    ) -> Error:
        conjunction = f" {validation.rules[0].conjunction} "
        elements = ErrorsGenerator.__get_elements_from_rules(validation)
    ...
    @staticmethod
    def _generate_only_one_of_error(
        validation: ValidationCondition, location: str
    ) -> Error:
        conjunction = f" {validation.rules[0].conjunction} "
        elements = ErrorsGenerator.__get_elements_from_rules(validation)
    ...
    def __get_elements_from_rules(validation: ValidationCondition) -> list:
        return [
            f'{el.element}{CONDITIONS_CONVERSIONS[el.condition] + el.value if el.value else ""}'
            for el in validation.rules[0].conditions
        ]

    @staticmethod
    def __get_elements_from_results(validation: ValidationCondition) -> list:
        return [
            f'{el.element}{RESULTS_CONVERSIONS[el.condition]}{el.value if el.value else ""}'
            for el in validation.results[0].conditions
        ]
    ...
    def __get_elements_from_conditions(validation: ValidationCondition) -> list:
        return [
            f'{el.element}{CONDITIONS_CONVERSIONS[el.condition]}{el.value if el.value else ""}'
            for el in validation.conditions[0].conditions
        ]
