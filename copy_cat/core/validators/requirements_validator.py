...
class RequirementsValidator(AbstractValidator):
    ...
    def validate(self, design_object: DesignObject, test_data: List[DataObject]) -> None:
    ...
    def _validate_children(self, children: List[DesignObject], test_data: List[DataObject], active_paths) -> None:
    ...
    def _validate_conditions(self, design_object: DesignObject, test_data: List[DataObject]) -> None:
        for c_type, conditions in self._get_grouped_validation_conditions(design_object.validation).items():
            try:
    ...
    def _validate_record_requirements(self, design_object: DesignObject, test_data: List[DataObject]) -> None:
        if design_object.children and not get_test_data_object(test_data, design_object.location):
    ...
    def _validate_requirements(self, design_object: DesignObject, test_data: List[DataObject]) -> None:
        if design_object.parent.skip_children:
            pass
        elif not design_object.children and not get_test_data_object(test_data, design_object.location):
    ...
    def _is_eligible_for_validation(self, design_object: DesignObject, active_paths: List) -> bool:
        if design_object.visible:
            self.result_parent = False
            self.result_children = False
            self._check_parent(design_object, active_paths)
            self._check_children(design_object, active_paths)
            return self.result_parent and (self.result_children or design_object.parent.min_occurs > '0') \
    ...
    def _check_parent(self, design_object: DesignObject, active_paths: List) -> None:
        if parent := design_object.parent:
            if parent.min_occurs or '/'.join(parent.location.split('/')[:-1]) in active_paths:
