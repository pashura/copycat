...
class ParcelService:
    ...
    def __token(self):
        if self.__Token is None:
            ssm = SSMService()
            self.environment = self.environment if self.environment in ['preprod', 'prod'] else 'prod'
            self.__Token = ssm.get_secret(f'/tpd/webxd/xd-server/{self.environment}/identity.token')
        return self.__Token

    def get_parcel_data(self, uid):
        headers = {'Authorization': 'Bearer ' + self.__token}
        url = f'{self.url}/{uid}/data'
        resp = requests.get(url, headers=headers, timeout=(CONNECT_TIMEOUT_SEC, READ_TIMEOUT_SEC), stream=True)
