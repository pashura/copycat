...
class LengthValidator:
    ...
    def validate_length(self, design_object, test_data_object):
        ...
        if int(restriction_obj.get('maxLength', IMPOSSIBLY_HUGE_LENGTH)) < int(test_data_object.get('length')):
            # TODO: Fix error message to be similar to web xd
            expected_length = design_object.get('attributes', [])[0].get('maxLength')
            error_message = f"Given data: '{test_data_object['text']}' with length {test_data_object['length']} " \
