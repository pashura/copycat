...
#
#     def validate(self, design_object: dict, test_data: list[DataObject], active_paths):
...
#         if design_object.get('validation') and self._is_eligible_for_validation(design_object, active_paths):
#             for cond_type, conditions in self._get_grouped_validation_conditions(design_object['validation']).items():
#                 try:
#                     validator = validation_condition_factory(cond_type)
#                     validator.validate(conditions, test_data, design_object['location'])
#                 except UnsupportedValidationConditionTypeError:
#                     print(f'Conditions {cond_type} skipped')
#
#     def _validate_requirements(self, design_object: dict, test_data: list[DataObject], active_paths):
#         if self._is_eligible_for_validation(design_object, active_paths):
