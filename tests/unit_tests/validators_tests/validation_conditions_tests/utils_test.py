...
@patch('copy_cat.validators.validation_conditions.utils.get_test_data_object')
@patch('copy_cat.validators.validation_conditions.utils.validate_condition')
def test_get_successful_conditions_count(mock_validate_condition, mock_get_test_data_object):
    validation_condition = ValidationCondition(**VALIDATION_CONDITION_IF_THEN_WITH_AND_CONJUNCTION)
    test_data = []
    location = 'location'
    mock_get_test_data_object.side_effect = [False, True, False, True]
    mock_validate_condition.side_effect = [True, False]
    actual = get_successful_conditions_count(validation_condition.results[0].conditions, test_data, location)
    expected = 1
