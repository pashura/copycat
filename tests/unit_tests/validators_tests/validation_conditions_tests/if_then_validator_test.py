...
class TestIfThenValidator:
    def setup_method(self):
        ...
        self.validation_condition_or = ValidationCondition(
            **VALIDATION_CONDITION_IF_THEN_WITH_OR_CONJUNCTION
        )
        self.validation_condition_and = ValidationCondition(
            **VALIDATION_CONDITION_IF_THEN_WITH_AND_CONJUNCTION
        )

    @patch.object(IfThenValidator, "_conditions_satisfied")
    @patch(
        "copy_cat.core.validators.validation_conditions.if_then_validator.ErrorsGenerator.generate"
    )
    ...
    @patch(
        "copy_cat.core.validators.validation_conditions.if_then_validator.get_successful_conditions_count"
    )
    def test__conditions_satisfied__or_returns_true_when_count_is_not_zero(
        ...
        actual = self.validator._conditions_satisfied(
            conditions, self.test_data, self.location
        )
        expected = True
    ...
    def test__conditions_satisfied__or_returns_false_when_count_is_zero(
        self, mock_get_successful_conditions_count
    ):
