...
def test_get_blank_design_success():
    with requests_mock.mock() as mock_:
        mock_.get(
            f"{GET_BLANK_DESIGN_URL}/{FILE_TYPE}/{VERSION}/{FILE_NAME}.json",
            json={"design": "design"},
            status_code=200,
        )
...
def test_search_design_success():
    with requests_mock.mock() as mock_:
        mock_.get(
            "{0}/{1}.json".format(SEARCH_DESIGN_URL, FILE_NAME),
            json={},
            status_code=200,
        )
        with patch.object(SSMService, "get_secret", new=new_get_key):
...
def test_get_design_success():
    with requests_mock.mock() as mock_:
        mock_.get(
            f"{GET_DESIGN_URL}/{ORGANIZATION_ID}/Designs/{DESIGN_NAME}.json",
            json={},
            status_code=200,
        )
        with patch.object(SSMService, "get_secret", new=new_get_key):
...
def test_get_reversed_design_success():
    with requests_mock.mock() as mock_:
        mock_.get(
            f"{GET_DESIGN_URL}/{ORGANIZATION_ID}/Designs/{DESIGN_NAME}.json/reversed",
            json={},
            status_code=200,
        )
        with patch.object(SSMService, "get_secret", new=new_get_key):
